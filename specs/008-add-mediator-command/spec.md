...
**Independent Test**: Can be tested independently by verifying that the command adds the mediator module to `nfw.yaml`, renders generator files, and preserves workspace state on failure. The test delivers value by ensuring developers can reliably add mediator capabilities to their services.

**Acceptance Scenarios**:

1. **Given** the nfw CLI is installed in a workspace with at least one service, **When** I run `nfw add mediator --service TestService --no-input`, **Then** the mediator module is added to the TestService configuration in `nfw.yaml`, generator files are rendered in the service directory, and the command completes successfully.
...
5. **Given** the nfw CLI is installed and I have a service with mediator generators configured, **When** the generator execution fails after updating `nfw.yaml`, **Then** the `nfw.yaml` changes are rolled back, preserving the original state with all comments intact.
...
- **FR-007**: The command MUST execute generator rendering for the mediator module using the configured generator engine and generators.

- **FR-008**: The command MUST complete execution within 5 seconds for typical workspaces with standard generator complexity.
...
- **FR-010**: The command MUST roll back any changes made to `nfw.yaml` if generator execution fails, ensuring atomic operation.
...
- **FR-013**: The command MUST use the generator context to resolve generator roots and configurations for the mediator module.

- **FR-014**: The command MUST construct generator parameters including service name, namespace, and workspace context for generator rendering.
...
- **FR-018**: The command MUST use the ArtifactGenerationService for workspace operations, generator execution, and service module registration.
...
- **AddMediatorCommandHandler**: A command handler that orchestrates the mediator addition workflow including workspace context loading, generator execution, and service module registration.
...
- **SC-008**: Generator execution MUST use the configured generator engine and produce valid output files in the target service directory.
...
- Generator sources are configured and accessible, with the mediator generator available in the configured generator repository.
...
- **Q**: What happens if the mediator module is already added to the service? → **A**: The command should detect this and report that the mediator module is already present, avoiding duplicate entries and redundant generator execution.
...
- **Q**: What generator parameters are required for mediator module rendering? → **A**: The command provides at minimum the service name, namespace, and workspace context. Additional parameters may be added as required by the generator configuration.

- **Q**: Should the command validate the generated code after generator execution? → **A**: Generator validation is the responsibility of the generator engine. The command ensures generator execution completes successfully but does not perform additional code validation.

- **Q**: How does the command handle different service types (e.g., .NET, Go, Rust)? → **A**: The command is generator-agnostic and delegates service-specific logic to the generator system. Different service types use different generators configured in the workspace.
...
- **Generator creation**: This command does not create or modify mediator generators. Generator management is handled separately through the generator system.
