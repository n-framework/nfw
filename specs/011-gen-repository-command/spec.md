...
**Independent Test**: Can be tested by running the command with a valid existing entity, verifying that the CLI reads generator configuration and generates all specified files in the correct locations.
...
1. **Given** a valid nfw project with an existing `User` entity in the default feature folder and persistence configured in `nfw.yaml`, **When** I run `nfw gen repository User`, **Then** the CLI:
   - Reads the repository generator configuration from `src/nfw-generators/`
   - Generates all files specified in the generator configuration
...
**Independent Test**: Can be tested by running the command with `--feature` flag, verifying the CLI applies the generator configuration for the specified feature.
...
1. **Given** a valid nfw project with a `payments` feature containing an `Order` entity and persistence configured, **When** I run `nfw gen repository Order --feature payments`, **Then** the CLI:
   - Reads the repository generator configuration
   - Generates all repository files in the `payments` feature's directory structure as specified by the generator
   - Completes in <2 seconds
...
- **Generator configuration not found**: When the repository generator configuration cannot be found in the generator store, the CLI returns an error to stderr and exits.
...
- **FR-003**: The CLI MUST read the repository generator configuration from `src/nfw-generators/` to determine which files to generate and their target locations.
- **FR-004**: The CLI MUST generate all files specified in the repository generator configuration with proper entity name substitution.
- **FR-005**: The CLI MUST validate that the specified entity exists in the target feature folder before reading generator configuration; if not found, it MUST return an error to stderr and exit with non-zero code.
...
- **FR-009**: The CLI MUST use the generator configuration to determine file paths, class names, and other generation parameters (the generator configuration defines these, not the CLI code).
...
- **Repository Generator Configuration**: Configuration file in `src/nfw-generators` that defines which files to generate, their target paths, and generator parameters. The CLI reads this configuration.
...
- **Generator Store**: The `src/nfw-generators` directory containing generator configurations and file generators that the CLI reads from.
...
- **SC-003**: 100% of generated files match the specifications defined in the repository generator configuration.
- **SC-004**: 100% of generated files are placed in the correct locations as defined by the generator configuration.
- **SC-005**: 100% of integration tests pass, covering all user scenarios, edge cases, and validation rules (including `nfw.yaml` persistence check and generator configuration reading).

## Assumptions

- nfw CLI handles file placement, DI registration, and generator processing based on generator configuration.
- Repository generator configuration in `src/nfw-generators` defines all file generation details (paths, substitutions, etc.).
...
- The generator store (`src/nfw-generators`) is accessible and contains valid repository generator configuration.

## Dependencies

- Existing nfw CLI infrastructure for command parsing (`clap`), generator reading (`serde`/`serde_yaml`), and file generation.
- `src/nfw-generators` repository containing repository generator configuration and file generators that the CLI reads from.
- `src/core-persistence-dotnet` module: Must be accessible for validation (generator rendering will reference it).
...
- Generator configuration defines all file generation steps, target paths, and substitutions that the CLI applies.

## Clarifications

- Q: What generator configuration parameters are available for repository generation? → A: The generator configuration in `src/nfw-generators` defines file generators, target paths (using placeholders like `{Feature}`, `{Entity}`), and substitution patterns. The CLI reads and applies this configuration.
...
- Q: Where is DI registration handled? → A: The generator configuration specifies DI registration details. The CLI applies this configuration to inject the registration into the appropriate file.

## Non-Goals

- The CLI will not hardcode file paths, class names, or layer conventions; these come from generator configuration.
...
- The CLI will not proceed if the repository generator configuration cannot be found in the generator store.
