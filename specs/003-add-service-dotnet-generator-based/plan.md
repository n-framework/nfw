# Implementation Plan: Generator-Based `nfw add service`

**Branch**: `feat/add-service-generator-based` | **Date**: 2026-04-04 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `src/nfw/specs/003-add-service-dotnet-generator-based/spec.md`

## Summary

Implement `nfw add service <name>` as a generator-first workflow that requires generator selection in automation, prompts in interactive mode, renders service generators from `nfw-generators` into `src/<ServiceName>/`, and records generator provenance in `nfw.yaml`.

## Technical Context

**Language/Version**: Rust 1.85+ (2024 edition)
**Primary Dependencies**: clap (CLI parsing), serde + serde_yaml (workspace metadata read/write), semver (generator version resolution), existing generator catalog/cache components from spec 001
**Storage**: File system (workspace service artifacts), workspace `nfw.yaml`, local generator cache
...
**Constraints**: Output path fixed to `src/<ServiceName>/`; `--generator` required in non-interactive mode; only generators with `type=service`; cleanup on interruption/failure; no separate service templating subsystem
**Scale/Scope**: One service generation per invocation; initial service baseline uses Domain/Application/Infrastructure/Api layers provided by the selected service generator
...
1. Generator selection policy for interactive vs non-interactive execution
2. Canonical service output location and collision behavior
3. Service-generator eligibility and validation boundary
4. Provenance persistence model in `nfw.yaml`
5. Layer dependency enforcement strategy from generator output to validation guarantees
...
```text
src/nfw/specs/003-add-service-dotnet-generator-based/
├── plan.md
├── research.md
├── data-model.md
├── quickstart.md
...
└── specs/
    ...
    └── 003-add-service-dotnet-generator-based/
```

**Structure Decision**: Reuse existing generator system and CLI composition boundaries. Add service-generation command handling in application command flow, with rendering and write operations in infrastructure services.
...
2. Generator selection/prompting and non-interactive validation
3. Service generator rendering into `src/<ServiceName>/`
