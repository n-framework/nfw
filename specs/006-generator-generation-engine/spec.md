# Feature Specification: Generator Generation Engine (Generic Execution)

## User Scenarios & Testing

### User Story 1 - Multi-step Generator Execution (Priority: P1)

As a generator author, I want to define a series of generation steps (rendering files, injecting code) in a configuration file so that the CLI can execute complex scaffolding logic without needing hardcoded workflows.
...
**Independent Test**: Create a generator with a `nfw.generator.yaml` containing multiple `render` steps and verify `nfw` executes all of them accurately.

**Acceptance Scenarios**:

1. **Given** a generator with a `nfw.generator.yaml` defining three `render` steps, **When** I run the corresponding generation command, **Then** all three files are created at their target locations with the correct content.
...
**Independent Test**: Use a generator with an `inject` step targeting a specific class and verify the new code appears inside the class body.

**Acceptance Scenarios**:

1. **Given** a target file with a class `public class Startup`, **When** I run a generation step with `action: inject` and `target: class(Startup)`, **Then** the generator content is appended to the class body.
...
- **FR-001**: The CLI MUST look for a `nfw.generator.yaml` file inside the resolved generator directory to determine execution steps.
- **FR-002**: The `nfw.generator.yaml` MUST support a `steps` list.
- **FR-003**: Each step MUST support an `action` field. Supported actions are `render`, `render_folder`, and `inject`.
- **FR-004**: The `render` action MUST take a `source` (single generator file) and a `destination` (output path).
...
- **FR-006**: The `injection_target` MUST support several modes:
  ...
  - `class(Name)`: Inside the body of the specified class (Note: Not yet implemented, returns error).
  - `function(Name)`: Inside the body of the specified function (Note: Not yet implemented, returns error).
  - `region(Name)`: Inside the specified region marker (e.g. `// region: Name`).
- **FR-007**: The CLI MUST use the **Tera** generator engine for both path interpolation and file content rendering.
...
- **Generator Step**: A single unit of work (render or inject) defined in the generator configuration.
...
- **SC-001**: Generator execution is purely data-driven; no new Rust code is required in `nfw` to support a new generator structure.
...
- Supporting complex conditional logic *inside* `nfw.generator.yaml` (should be handled within Tera generators where possible).
