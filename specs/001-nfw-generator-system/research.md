# Research: Generator Metadata Schema, Discovery, and Versioning
...
- Core's `GeneratorMetadata` uses plain Rust structs (no serde derives)
...
- Common convention in generator systems
...
1. **Default**: Use cached generators without remote check
2. **Explicit refresh**: User runs `nfw generators --refresh` to fetch latest
3. **Auto-refresh on error**: If cache is corrupted, auto-fetch from remote
4. **Stale warning**: When using cached generators, display "last refreshed" timestamp

**Rationale**:

- Fastest UX (no network latency on normal operations)
- Deterministic behavior (generators don't change unexpectedly)
- Explicit control for users who need freshness
- Aligns with spec assumption: "Generator discovery is an explicit operation"
...
- Consistent with project preference (`nfw.yaml`, `nfw.generator.yaml`)
...
- Cache: `~/.nfw/generators/` (Linux/macOS) or `%USERPROFILE%\.nfw\generators\` (Windows)
...
- `GeneratorRegistry` depends on `GitRepository`, `YamlParser`, `FileSystem`, etc.
...
### Generator Metadata Filename

**Decision**: `nfw.generator.yaml` (not `generator.json`)

**Rationale**:

- Consistent with YAML preference throughout project
- More human-readable/editable for generator authors
...
- Example: `https://github.com/org/nfw-generators.git` → `nfw-generators`
...
**Decision**: Flat structure with source-named subdirectories in `~/.nfw/generators/`

```bash
~/.nfw/generators/
├── official/           # Cloned from github.com/n-framework/nfw-generators
├── my-company/         # Cloned from custom source
└── .registry.json      # Index of cached generators
```
...
1. Parse all generator versions from metadata (via VersionComparator)
