...
**Independent Test**: Can be tested independently by verifying that the command adds the persistence module to `nfw.yaml`, renders generator files for DbContext and repository base classes, and preserves workspace state on failure. The test delivers value by ensuring developers can reliably add persistence capabilities to their services.

**Acceptance Scenarios**:

1. **Given** the nfw CLI is installed in a workspace with at least one service, **When** I run `nfw add persistence --service TestService --no-input`, **Then** the persistence module is added to the TestService configuration in `nfw.yaml`, generator files are rendered in the service directory including DbContext and repository base classes, and the command completes successfully.
...
5. **Given** the nfw CLI is installed and I have a service with persistence generators configured, **When** the generator execution fails after updating `nfw.yaml`, **Then** the `nfw.yaml` changes are rolled back, preserving the original state with all comments intact.

6. **Given** a service with the persistence module already added, **When** I run `nfw add persistence` for the same service, **Then** the command detects the existing persistence module and reports that it is already present, avoiding duplicate entries and redundant generator execution.
...
2. **Given** a service receiving the persistence module, **When** generator execution completes, **Then** repository base classes are generated with generic CRUD methods (add, update, delete, getById, getAll, query with pagination).
...
**Note**: This is an acceptance scenario of US1, not a separate user story. Database configuration generation is part of the generator rendering process when adding persistence.
...
1. **Given** a service receiving the persistence module, **When** generator execution completes, **Then** configuration artifacts are generated with database connection settings following the framework's configuration pattern.
...
3. **Given** different database providers, **When** persistence generators are executed, **Then** the generated configuration supports the provider specified in the service generator (e.g., SQL Server, PostgreSQL, SQLite).
...
- **Generator execution failure**: When generator rendering fails (missing generator file, invalid generator syntax, permission errors), the command must roll back any changes made to `nfw.yaml` and report the specific failure reason.
...
- **Missing generator directory**: When the configured generator directory does not exist or is inaccessible, the command must fail with a specific error about generator resolution.

- **Persistence module already present**: When the persistence module is already registered in the target service, the command must detect this and report that the module is already present without attempting to add it again or re-execute generators.
...
- **Missing database provider**: When the service generator does not specify a database provider, the command must use a sensible default or prompt the user to select one.
...
- **FR-007**: The command MUST complete execution within 5 seconds for typical workspaces with standard generator complexity.
...
- **FR-009**: The command MUST roll back any changes made to `nfw.yaml` if generator execution fails, ensuring atomic operation.
...
- **FR-012**: The command MUST use the generator context to resolve generator roots and configurations for the persistence module.

- **FR-013**: The command MUST construct generator parameters including service name, namespace, and workspace context for generator rendering.
...
- **FR-017**: The command MUST use the ArtifactGenerationService for workspace operations, generator execution, and service module registration.
...
- **AddPersistenceCommandHandler**: A command handler that orchestrates the persistence addition workflow including workspace context loading, generator execution, and service module registration.
...
- **ArtifactGenerationService**: A service responsible for generator context loading, workspace operations, generator engine execution, and service module registration.

- **GeneratorParameters**: A value object containing generator rendering parameters including name, namespace, and service information.

- **WorkspaceContext**: A value object containing workspace configuration, root path, and parsed `nfw.yaml` content.

- **ServiceInfo**: A value object representing a service with its name, path, and generator configuration.
...
- **SC-003**: The command MUST complete execution within 5 seconds for workspaces with up to 10 services and standard generator complexity.

- **SC-004**: The command MUST roll back `nfw.yaml` changes within 1 second when generator execution fails, ensuring no partial state is left.
...
- **SC-008**: Generator execution MUST use the configured generator engine and produce valid output files including DbContext, repository base classes, and configuration artifacts in the target service directory.
...
- Generator sources are configured and accessible, with the persistence generator available in the configured generator repository.
...
- The generator engine is properly configured and can resolve generator roots and execute generator steps.

- The ArtifactGenerationService handles workspace operations, generator context loading, and service module registration.
...
- The service generator specifies or defaults to a database provider (e.g., SQL Server, PostgreSQL, SQLite).
...
- The nfw-generators repository includes the persistence generator at `generators/dotnet-service/persistence/` with nfw.generator.yaml configuration and .tera generator files for DbContext, RepositoryBase, and configuration generation.
...
- **Generator system**: The command depends on the generator engine (`GeneratorEngine` trait) and generator root resolver (`GeneratorRootResolver` trait) for generator execution.
...
- **core-persistence-dotnet**: The persistence generators reference abstractions and patterns defined in the NFramework.Persistence package.

- **nfw-generators**: The command requires the persistence generator to be available at `generators/dotnet-service/persistence/` in the configured generator sources with nfw.generator.yaml and .tera files for code generation.
...
- **Q**: What happens if the persistence module is already added to the service? → **A**: The command should detect this and report that the persistence module is already present, avoiding duplicate entries and redundant generator execution.
...
- **Q**: How should the command handle custom generator sources? → **A**: The command uses the configured generator sources from `nfw.yaml` and the generator root resolver, supporting both local and remote generator repositories as configured by the workspace.

- **Q**: What generator parameters are required for persistence module rendering? → **A**: The command provides at minimum the service name, namespace, and workspace context. Additional parameters may include database provider type, connection string placeholders, and entity configuration paths as required by the generator configuration.

- **Q**: How does the command handle different database providers? → **A**: The command is generator-agnostic and delegates provider-specific logic to the generator system. Different database providers use different generators configured in the workspace.

- **Q**: Should the command validate the generated code after generator execution? → **A**: Generator validation is the responsibility of the generator engine. The command ensures generator execution completes successfully but does not perform additional code validation beyond checking for compilation errors if specified by the generator.

- **Q**: What happens if the service project doesn't reference Entity Framework Core? → **A**: The command must fail with a clear error indicating that Entity Framework Core is required for the persistence module. The generator system may optionally add the necessary package references as part of generator execution.
...
- **Generator creation**: This command does not create or modify persistence generators. Generator management is handled separately through the generator system.

- **Service creation**: This command does not create new services. Service creation is handled by the `nfw add service` command.

- **Code validation**: This command does not perform post-generation code validation beyond ensuring generator execution succeeds.

- **Dependency management**: This command does not manage package dependencies or project files beyond what the generators render.
...
- **Generator debugging**: This command does not provide generator debugging capabilities. Generator issues should be debugged using dedicated generator development tools.
...
- **Interactive generator customization**: This command does not support interactive customization of generator parameters. All customization is done through generator configuration and command parameters.
