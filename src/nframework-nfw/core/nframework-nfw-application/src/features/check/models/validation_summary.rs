...
use crate::features::check::models::exit_outcome::ExitOutcome;
...
impl ValidationSummary {
    pub fn from_findings(findings: &[ValidationFinding], interrupted: bool) -> Self {
        ...
        Self {
            total_findings,
            project_reference_count: *grouped_counts
                .get(&FindingType::ProjectReference)
                .unwrap_or(&0),
            namespace_usage_count: *grouped_counts
                .get(&FindingType::NamespaceUsage)
                .unwrap_or(&0),
            package_usage_count: *grouped_counts.get(&FindingType::PackageUsage).unwrap_or(&0),
            unreadable_artifact_count: *grouped_counts
                .get(&FindingType::UnreadableArtifact)
                .unwrap_or(&0),
