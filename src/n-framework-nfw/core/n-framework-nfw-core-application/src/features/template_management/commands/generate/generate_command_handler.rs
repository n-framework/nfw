    ...
    E: TemplateEngine,
{
    ...
    fn resolve_template_root(
        ...
        generator_type: &str,
        workspace_root: &Path,
    ) -> Result<std::path::PathBuf, GenerateError> {
        let template_id = self.extract_template_id(nfw_yaml, generator_type)?;
        self.root_resolver
            .resolve(nfw_yaml, template_id, workspace_root)
    ...
    fn load_nfw_yaml(&self, workspace_root: &Path) -> Result<YamlValue, GenerateError> {
        ...
        let content = fs::read_to_string(&nfw_yaml_path)
            .map_err(|e| GenerateError::WorkspaceError(format!("failed to read nfw.yaml: {e}")))?;
    ...
    fn load_and_validate_template_config(
        ...
        config
            .validate()
            .map_err(|e| GenerateError::ConfigError(format!("template validation failed: {e}")))?;
        Ok(config)
    }
        ...
        let namespace = nfw_yaml
            ...
            .ok_or_else(|| {
                GenerateError::ConfigError(
                    "missing 'workspace.namespace' in nfw.yaml. this is required for generation."
                        .to_string(),
                )
            })?;
