...
use crate::features::service_management::services::abstractions::service_generator_renderer::ServiceGeneratorRenderer;
use crate::features::service_management::services::abstractions::service_generator_selector::ServiceGeneratorSelector;
...
use crate::features::service_management::services::service_generator_provenance_service::ServiceGeneratorProvenanceService;
    ...
    PS: ServiceProvenanceStore,
{
    ...
    renderer: R,
    provenance_service: ServiceGeneratorProvenanceService<PS>,
    ...
    PS: ServiceProvenanceStore,
{
    ...
    pub fn prepare_generation_plan(
        ...
        // Load nfw.yaml to get local generator sources
        ...
        let generator_resolution = self.input_resolution_service.resolve_generator_selection(
            &request,
            &workspace_root,
            &nfw_yaml,
        )?;
        ...
        self.plan_builder
            .build(&service_name, &workspace_root, &generator_resolution)
    }
    ...
    pub fn execute_generation_plan(
        ...
        if let Err(error) = self.provenance_service.persist(
            ...
            &plan.service_name,
            &plan.generator_id,
            &plan.generator_version.to_string(),
        ) {
            return Err(self.cleanup_and_wrap(&plan.output_root, error));
        }
        ...
        Ok(AddServiceCommandResult {
            ...
            generator_version: plan.generator_version.to_string(),
        })
    }
