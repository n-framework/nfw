...
struct TrackingConfigStore {
    sources: Arc<Mutex<Vec<GeneratorSource>>>,
}

impl TrackingConfigStore {
    fn new(sources: Vec<GeneratorSource>) -> Self {
        Self {
...
impl GeneratorCatalogSource for MockCatalogSource {
    ...
    fn read_generator_metadata(&self, generator_directory: &Path) -> Result<String, String> {
        self.metadata_by_directory
            ...
            .ok_or_else(|| {
                format!(
                    "generator metadata is not configured for '{}'",
...
impl GeneratorSourceSynchronizer for TrackingSourceSynchronizer {
...
#[test]
fn lists_generators_from_single_source() {
    let root = PathBuf::from("/tmp/nfw/source-official");
    let generator_path = root.join("web-api");
    let (service, _, _) = create_service(
        ...
        HashMap::from([(root, vec![generator_path.clone()])]),
        HashMap::from([(generator_path, valid_generator_yaml("web-api", "Web API"))]),
        HashMap::new(),
    );
    ...
    assert_eq!(generators[0].id, "web-api");
    assert_eq!(generators[0].source_name, "official");
}
...
fn adds_source_when_url_is_valid() {
    let (service, store, _) = create_service(
        vec![GeneratorSource::new(
            "official".to_owned(),
            "https://github.com/n-framework/nfw-generators".to_owned(),
        )],
        HashMap::new(),
        HashMap::new(),
        HashMap::new(),
...
type TestGeneratorsService = GeneratorsService<
...
fn create_service(
    ...
    let service = GeneratorsService::new(
        synchronizer.clone(),
        catalog_resolver,
        store.clone(),
        TestValidator,
