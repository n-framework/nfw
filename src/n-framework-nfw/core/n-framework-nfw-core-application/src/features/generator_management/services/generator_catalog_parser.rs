...
use crate::features::generator_management::models::errors::GeneratorCatalogError;
...
#[derive(Debug, Clone)]
pub struct GeneratorCatalogParser<Y, V, C>
where
    Y: YamlParser,
    V: Validator,
    C: VersionComparator,
...
impl<Y, V, C> GeneratorCatalogParser<Y, V, C>
where
    Y: YamlParser,
    V: Validator,
    C: VersionComparator,
{
    ...
    pub fn parse_generator_metadata(
        &self,
        yaml_content: &str,
    ) -> Result<GeneratorMetadata, GeneratorCatalogError> {
        let raw = self
            .yaml_parser
            .parse::<RawGeneratorMetadata>(yaml_content)
            .map_err(GeneratorCatalogError::InvalidYaml)?;
        ...
        metadata
            .validate()
            .map_err(|error| GeneratorCatalogError::InvalidField {
                field: error.field(),
...
where
    ...
    C: VersionComparator,
{
    fn validate_and_parse_id(&self, id: Option<String>) -> Result<String, GeneratorCatalogError> {
        let id = required_non_empty(id, "id")?;
        if !self.validator.is_kebab_case(&id) {
            return Err(GeneratorCatalogError::InvalidField {
                field: "id",
    ...
    ) -> Result<Version, GeneratorCatalogError> {
        let version = required_non_empty(version, "version")?;
        self.version_comparator.parse(&version).map_err(|error| {
            GeneratorCatalogError::InvalidField {
                field: "version",
                reason: error,
            }
        })?;
        Version::from_str(&version).map_err(|error| GeneratorCatalogError::InvalidField {
            field: "version",
            reason: error.to_string(),
        })
    }
    ...
    ) -> Result<Option<Version>, GeneratorCatalogError> {
    ...
    ) -> Result<Option<String>, GeneratorCatalogError> {
    ...
    ) -> Result<Language, GeneratorCatalogError> {
...
fn required_non_empty(
    value: Option<String>,
    field: &'static str,
) -> Result<String, GeneratorCatalogError> {
    let value = value.ok_or(GeneratorCatalogError::MissingField { field })?;
    let normalized = value.trim();

    if normalized.is_empty() {
        return Err(GeneratorCatalogError::EmptyField { field });
    }
...
fn parse_language(value: String) -> Result<Language, GeneratorCatalogError> {
    match value.as_str() {
        "neutral" => Ok(Language::Neutral),
        "dotnet" | "csharp" | "c#" => Ok(Language::Dotnet),
        "go" => Ok(Language::Go),
        "rust" => Ok(Language::Rust),
        _ => Err(GeneratorCatalogError::UnsupportedLanguage { value }),
    }
}

#[cfg(test)]
#[path = "generator_catalog_parser.tests.rs"]
mod tests;
