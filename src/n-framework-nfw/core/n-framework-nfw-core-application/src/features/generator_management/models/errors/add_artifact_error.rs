...
pub enum AddArtifactError {
    ...
    ConfigError(String),
    /// The requested generator could not be found locally or in the cache.
    GeneratorNotFound(String),
    /// A custom parameter was malformed.
    InvalidParameter(String),
    /// The underlying generator engine reported a failure.
    ExecutionFailed(Box<GeneratorError>),
...
impl fmt::Display for AddArtifactError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::GeneratorNotFound(msg) => write!(f, "generator not found: {}", msg),
