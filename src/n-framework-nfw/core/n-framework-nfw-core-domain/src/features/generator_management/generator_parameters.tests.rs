...
fn with_name_sets_name() {
    let params = GeneratorParameters::new().with_name("MyService").unwrap();
...
fn with_feature_sets_feature() {
    let params = GeneratorParameters::new().with_feature("Auth").unwrap();
...
fn try_from_btreemap_validates_keys() {
    ...
    let result = GeneratorParameters::try_from(map);
...
fn try_from_json_object_works() {
    ...
    let params = GeneratorParameters::try_from(val).unwrap();
...
fn try_from_json_non_object_fails() {
    use serde_json::json;
    assert!(GeneratorParameters::try_from(json!("string")).is_err());
    assert!(GeneratorParameters::try_from(json!([1, 2, 3])).is_err());
}
