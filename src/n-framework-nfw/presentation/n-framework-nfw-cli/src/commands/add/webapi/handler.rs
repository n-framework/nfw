...
use n_framework_nfw_core_application::features::generator_management::commands::add_webapi::add_webapi_command::{AddWebApiCommand, WebApiConfig};
        ...
        if let Err(e) = self.handler.handle(&command) {
            ...
            let actionable_msg = match &e {
                ...
                AddArtifactError::ExecutionFailed(err) => format!(
                    "Generator execution failed: {}. Check generator logs for details.",
                    err
                ),
                _ => e.to_string(),
            };
