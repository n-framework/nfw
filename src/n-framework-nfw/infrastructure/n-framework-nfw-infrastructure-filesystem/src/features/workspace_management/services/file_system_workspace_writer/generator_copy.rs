...
pub fn copy_generator_content(
    ...
    if !content_root.is_dir() {
        return Err(format!(
            "generator directory '{}' is missing required 'content/' directory",
            generator_root.display()
        ));
    }
...
fn copy_directory_recursive(
    ...
    for entry in fs::read_dir(current_path).map_err(|error| {
        format!(
            "failed to read generator content '{}': {error}",
            current_path.display()
        )
    })? {
        let entry = entry.map_err(|error| {
            format!(
                "failed to read a generator content entry in '{}': {error}",
                current_path.display()
            )
        })?;
        ...
        let relative_path = source_path.strip_prefix(content_root).map_err(|error| {
            format!(
                "failed to compute generator-relative path for '{}': {error}",
                source_path.display()
            )
        })?;
        ...
        let bytes = fs::read(&source_path).map_err(|error| {
            format!(
                "failed to read generator file '{}': {error}",
                source_path.display()
            )
        })?;
