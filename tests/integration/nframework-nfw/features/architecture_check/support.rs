...
pub fn add_project(
    ...
    package_references: &[&str],
    source_files: &[(&str, &str)],
) -> PathBuf {
    let project_dir = workspace_root.join(relative_project_dir);
    fs::create_dir_all(&project_dir).expect("project directory should be created");

    let project_file = project_dir.join(format!("{project_name}.csproj"));
    let project_references_xml = project_references
        .iter()
        .map(|value| format!(r#"<ProjectReference Include="{value}" />"#))
        .collect::<Vec<_>>()
        .join("\n    ");
    let package_references_xml = package_references
        .iter()
...
"
    ...
    for (relative_source_path, content) in source_files {
