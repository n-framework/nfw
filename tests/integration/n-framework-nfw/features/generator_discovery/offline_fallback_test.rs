//! Tests for offline/fallback behavior when generator sources are unreachable.
//!
//! These tests verify that when all sources are unreachable (network failure),
//! the system can fall back to cached generators and provide appropriate warnings.
...
impl GeneratorSourceSynchronizer for CachedWarningSynchronizer {
...
#[test]
fn uses_cached_generators_when_all_sources_unreachable() {
    ...
    create_generator_repository(&seed_repository, "offline-test");
    ...
    let real_synchronizer = GitGeneratorCatalogSource::new(git_repository, path_resolver);
    ...
    let (offline_generators, offline_warnings) = generators_service
        .list_generators()
        .expect("cached generators should be used when remote is unreachable");

    assert!(
        !offline_generators.is_empty(),
        "should still discover generators from cache"
    );
    assert!(
        offline_warnings
            .iter()
...
#[test]
fn warns_about_stale_data_when_using_cached_generators() {
    let sandbox = create_sandbox_directory();
    let cache_directory = sandbox.join("cache");
...
id: cached-generator
name: Cached Generator
description: A cached test generator
version: 1.0.0
language: rust
tags:
  - cached
"#;
    fs::write(cache_path.join("nfw.generator.yaml"), metadata_content)
        .expect("failed to write generator.yaml");
    ...
    let (generators, warnings) = generators_service
        .list_generators()
        .expect("cached generators should be listed with warnings");
    assert_eq!(generators.len(), 1, "should list cached generator");
    assert!(
        warnings
            .iter()
            .any(|warning| warning.contains("using cached data")),
