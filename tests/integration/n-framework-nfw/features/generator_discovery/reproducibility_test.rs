...
#[test]
fn generating_twice_from_same_generator_produces_identical_structure() {
    let sandbox = create_sandbox_directory();
    let generator_root = sandbox.join("generator");
    let output_a = sandbox.join("output-a");
    let output_b = sandbox.join("output-b");

    create_generator(&generator_root);
    ...
    generator
        .generate(&generator_root, &output_a, &context)
        .expect("first generation should succeed");
    generator
        .generate(&generator_root, &output_b, &context)
...
fn create_generator(generator_root: &Path) {
    let source_directory = generator_root.join("{{ServiceName}}");
    fs::create_dir_all(&source_directory).expect("generator directory should be created");
    ...
    .expect("generator project file should be written");
}
