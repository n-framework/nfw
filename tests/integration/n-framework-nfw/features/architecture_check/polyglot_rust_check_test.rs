...
fn check_detects_rust_forbidden_usage_and_references() {
    ...
    fs::create_dir_all(domain_dir.join("src"))
        .expect("rust domain src directory should be created");
    fs::write(
...
[dependencies]
nframework-application = { path = "../application" }
axum = "0.7"
"#,
    )
    ...
    let application_dir = workspace_root.join("src/application");
...
"#,
    ...
    fs::write(
        application_dir.join("src/lib.rs"),
